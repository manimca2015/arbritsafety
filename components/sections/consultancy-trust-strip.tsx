...
type TrustItem = {
  ...
  suffix?: string;
  label: string;
  /** Why this number matters to a consultancy buyer specifically. */
  detail: string;
};
...
const items: TrustItem[] = [
  {
    icon: Clock,
    count: statOf("Years of Experience", 20),
    suffix: "+",
    label: "Years of Experience",
    detail: "Advising on HSE across the Gulf",
  },
  {
    icon: Users,
    ...
    detail: "Dubai, Abu Dhabi & Saudi Arabia",
  },
  {
    icon: Award,
    count: statOf("Course Pass Rate", 98),
    suffix: "%",
    label: "Course Pass Rate",
    detail: "Competency that stands up to audit",
  },
  {
    icon: BookOpenCheck,
    ...
    detail: "To close any gap we find on site",
  },
];

/**
 * Divider borders per cell. The grid reflows 1 → 2 → 4 columns, so each cell
 * needs its rule declared per breakpoint: a stacked cell takes a top border,
 * a cell beside a sibling takes a left border, and the first cell of any row
 * takes neither.
 */
const cellBorders = [
  "",
...
function TrustCell({ item }: { item: TrustItem }) {
  const { icon: Icon, count, suffix, label, detail } = item;
...
export function ConsultancyTrustStrip() {
  return (
    <section aria-label="Why clients choose Arbrit" className="relative bg-white">
      <div
        aria-hidden="true"
      ...
      <Reveal className="relative mx-auto max-w-7xl px-6 pb-10 pt-8 sm:pb-14 sm:pt-10">
        <div className="overflow-hidden rounded-2xl border border-navy/10 bg-white shadow-[0_10px_30px_-18px_rgba(18,59,109,0.45)]">
          <div
            aria-hidden="true"
            className="h-1 bg-gradient-to-r from-[#0066b2] via-[#0066b2]/40 to-[#0066b2]"
          />

          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {items.map((item, i) => (
              <li key={item.label} className={`border-navy/10 ${cellBorders[i]}`}>
                <TrustCell item={item} />
              </li>
            ))}
          </ul>
